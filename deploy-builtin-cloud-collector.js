...
// Configuration
const FUNCTION_NAME = 'prod-star-collector-builtin';
const RULE_NAME = 'prod-star-collection-schedule-builtin';
const TARGET_ID = 'prod-star-collector-builtin-target';

// Create deployment package
async function createDeploymentPackage() {
  console.log('📦 Creating deployment package...');
  
  const output = fs.createWriteStream('lambda-star-collector-builtin.zip');
  ...
  return new Promise((resolve, reject) => {
    ...
    archive.file('lambda-star-collector-builtin.js', { name: 'index.js' });
    
    // Add AWS SDK (required for Lambda)
...
async function createIAMRole() {
  ...
  try {
    ...
    const createRoleParams = {
      ...
      Description: 'Role for star collection Lambda function (built-in)'
    };
...
async function createLambdaFunction(roleArn) {
  ...
  const zipBuffer = fs.readFileSync('lambda-star-collector-builtin.zip');
  ...
  const params = {
    ...
    Description: 'Star collection Lambda function (built-in modules only)',
    Timeout: 30,
    MemorySize: 128,
    Environment: {
      Variables: {
...
async function deploy() {
  try {
    console.log('🚀 Starting cloud deployment for star collection (built-in modules)...');
