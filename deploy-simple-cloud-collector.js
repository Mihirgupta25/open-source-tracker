...
// Configuration
const FUNCTION_NAME = 'prod-star-collector-simple';
const RULE_NAME = 'prod-star-collection-schedule-simple';
const TARGET_ID = 'prod-star-collector-simple-target';

// Create deployment package
async function createDeploymentPackage() {
  console.log('📦 Creating deployment package...');
  
  const output = fs.createWriteStream('lambda-star-collector-simple.zip');
  ...
  return new Promise((resolve, reject) => {
    ...
    archive.file('lambda-star-collector-simple.js', { name: 'index.js' });
    
    // Add only axios and its dependencies
    ...
    archive.directory('node_modules/form-data/', 'node_modules/form-data/');
...
async function createLambdaFunction(roleArn) {
  ...
  const zipBuffer = fs.readFileSync('lambda-star-collector-simple.zip');
  ...
  const params = {
    ...
    Description: 'Star collection Lambda function (simplified)',
    Timeout: 30,
    MemorySize: 128,
    Environment: {
      Variables: {
...
async function deploy() {
  try {
    console.log('🚀 Starting cloud deployment for star collection (simplified)...');
