...
// Configuration
const FUNCTION_NAME = 'prod-star-collector';
const RULE_NAME = 'prod-star-collection-schedule';
const TARGET_ID = 'prod-star-collector-target';

// Create deployment package
async function createDeploymentPackage() {
  console.log('📦 Creating deployment package...');
  
  const output = fs.createWriteStream('lambda-star-collector.zip');
  ...
  return new Promise((resolve, reject) => {
    ...
    archive.file('lambda-star-collector.js', { name: 'index.js' });
    
    // Add node_modules (only required packages)
    archive.directory('node_modules/aws-sdk/', 'node_modules/aws-sdk/');
    archive.directory('node_modules/axios/', 'node_modules/axios/');
    archive.directory('node_modules/jmespath/', 'node_modules/jmespath/');
    archive.directory('node_modules/querystring/', 'node_modules/querystring/');
    archive.directory('node_modules/url/', 'node_modules/url/');
    archive.directory('node_modules/url-parse/', 'node_modules/url-parse/');
    archive.directory('node_modules/qs/', 'node_modules/qs/');
    archive.directory('node_modules/follow-redirects/', 'node_modules/follow-redirects/');
    archive.directory('node_modules/proxy-from-env/', 'node_modules/proxy-from-env/');
    archive.directory('node_modules/tunnel/', 'node_modules/tunnel/');
...
async function createIAMRole() {
  ...
  try {
    ...
    const createRoleParams = {
      ...
      Description: 'Role for star collection Lambda function'
    };
...
async function createLambdaFunction(roleArn) {
  ...
  const zipBuffer = fs.readFileSync('lambda-star-collector.zip');
  ...
  const params = {
    ...
    Description: 'Star collection Lambda function',
    Timeout: 30,
    MemorySize: 128,
    Environment: {
      Variables: {
...
async function deploy() {
  try {
    console.log('🚀 Starting cloud deployment for star collection...');
