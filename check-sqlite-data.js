...
function checkSQLiteData() {
  console.log('🔍 Checking actual SQLite data...\n');
  
  try {
    // Check star growth
    console.log('📊 Star Growth Data (SQLite):');
    const starDb = new Database(dbPaths.starGrowth);
    const starData = starDb.prepare('SELECT * FROM stars ORDER BY timestamp').all();
    ...
    const prData = prDb.prepare('SELECT * FROM pr_ratios ORDER BY date').all();
    ...
    const issueData = issueDb.prepare('SELECT * FROM issue_ratios ORDER BY date').all();
