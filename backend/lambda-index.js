...
async function resetStagingData() {
  try {
    ...
    return { 
      success: true, 
      message: 'Staging data reset successfully (test mode)',
      itemsCleared: 0,
      clearedTables: {
        prVelocity: 0
      }
...
async function clearTable(tableName, repo) {
  ...
  if (result.Items.length > 0) {
    const deleteRequests = result.Items.map(item => {
      // Determine the correct key structure based on table name
      let key = { repo: item.repo };
      ...
      if (tableName.includes('star-growth')) {
        ...
        key.week_start = item.week_start;
      } else {
        // Default fallback
        key.timestamp = item.timestamp;
      }
    ...
    return result.Items.length; // Return the number of items cleared
  }
  return 0; // Return 0 if no items to clear
}
